...
void main() {
    ...
    if(pos.x < 16*5 && pos.y < 16*5 && pos.z == 0 && pos.w == 1) {
        //vec2 point = pos + ivec2(floor(Position.x), floor(Position.z)) / vec2(6,6);
        //color = vec4(voronoi3d(vec3(point.x / 5, point.y / 5, GameTime * 400.0)), 1);
        //float p = pNoise(pos + vec2(0, fract(GameTime * 1200)), 3);
        //color = vec4(p, p, p, 1);
        /*float Time = GameTime * 400;
        float n1 = pNoise(pos.xy * vec2(3,3), 1, 500.0, max(sin(Time), sin(Time * 3.0)) * 1.0 + 5.0);
        float n2 = pNoise(pos.xy * vec2(3,3) + vec2(0, cos(Time / 3.0) * 10.0), 1, 250.0, max(sin(Time + PI * 5.0), sin(Time * 3.0)) * 1.0 + 4.0);
        float c = n1 + n2;
        vec3 C = vec3(0.0, 0.0, 0.0);
    
        C = vec3(0.050, 0.050, 0.086);
        if(c > 0.8) C = vec3(0.588, 0.184, 0.890);
        if(c > 0.3 && c < 0.4) C = vec3(0.301,0.196,0.313);
        if(c > 0.1 && c < 0.2) C = vec3(0.203,0.098,0.294);
        color = vec4(C, 1);*/
